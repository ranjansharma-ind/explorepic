...
const Navbar = () => {
  ...
  return (
    <>
    <header className={`fixed w-full z-50 transition-all duration-500 ${scrolled ? "bg-white/90 shadow-xl" : "bg-transparent"}`}>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-x-4 top-20 bg-white/95 backdrop-blur-xl shadow-2xl shadow-navy/10 z-40 overflow-hidden border border-gray-100/50 lg:hidden"
          >
            <div className="flex flex-col p-6 space-y-2">
              {links.map((link) => (
                <a
                  key={link}
                  href={`#${link.toLowerCase().replace(" ", "-")}`}
                  className="text-navy/80 hover:text-customBlue hover:bg-gray-50 font-semibold text-lg py-3 px-4 rounded-2xl transition-colors"
              ...
              <div className="pt-6 mt-2 border-t border-gray-100 flex flex-col gap-3">
                <button className="w-full py-4 rounded-full bg-customYellow text-black font-semibold flex items-center justify-center gap-2 hover:bg-saffron transition-colors shadow-lg">
                  <User size={18} /> Login
                </button>
              </div>
            </div>
